...
c	================================================================
c     Modifications introduced by Magnus Gustafsson in 2023
c     to make the program run.
c     Further modifications to compute absorption with given para-H2 fractions.
c     ================================================================
c     Modifications to remove compilation warnings were made by Glenn Orton
c     on 2024 June 25.
c     ================================================================
c	Program (ver.1) based on:
c	A. Borysow, Icarus, vol.92, 273 (1991).
c	This is an updated version (ver. 2), see erratum:
c	"Erratum to: Rotovibrational collision-induced 
c	absorption spectra of {H$_2$-H$_2$} pairs  in the 
c	fundamental band $(0 \rightarrow 1)$ at temperatures 
c	from 20 to 300~{K}, {$Icarus$}, {\bf 92}, 273--279, (1991) ",
c	author = " A. Borysow" , journal="Icarus", 
c	year = "1993", pages = "614 ", volume = "106.
c	=================================================================
c	 NOTE:
c        An error has been found in the  program modeling 
c	 the collision induced spectra (1991)
c	 of hydrogen pairs in the fundamental band.
c	 The slight mistake affects mostly the spectra at higher 
c	 temperatures (T>200K), mainly near the centers of the 
c	 roto-translational lines.
c	 This version of the program (ver. 2) accounts for bound states as 
c	 well as for free states, and fully reconstructs the quantum 
c	 mechanical lineshape computations reported before 
c	 (Meyer, Borysow, Frommhold, PRA,40,6931,1990).
c	 The temperature range has been extended to include T=350K (max), 
c	 in order to facilitate modeling of Jupiter's atmosphere.
c	=================================================================
c	Program written by: Aleksandra Borysow
c	Niels Bohr Institute for Astronomy, Physics and Geophysics,
c	University Observatory
c	Juliane Maries vej 39
c	DK-2100 Copenhagen
c	Denmark
c	e-mail: aborysow@nbi.dk
c       Modification by Magnus Gustafsson, 2023 November
c	***********************************************************
c	Program to model H2-H2 CIA spectra at low temperatures
c	for planetary interest (20-350K)
c	NOTE: low resolution model, bound states included (1993)
c	================================================================
c
c	Program asks interactively for:
c	TEMPERATURE [K], choose from 20 - 350K 
c	min. frequency (FREQLO) (in cm^[-1])
c	max. frequency (FREQMAX);
c	frequency interval (in cm^{-1})
c	Max. number of points: 600 (look for "NF" in the program)
c
c	Program computes absorption spectrum at temperatures
c	between  20 and 350K H2-H2 fundamental band
...
c
      ...
      COMMON/SPEKTRM/ FREQLO, DFREQ, FMAX, NTYPE, 
     1      Like, nvib_F, nvib_I
      common /wagi/ w(2)
...
c
C     NTYPE=0 for equilibrium hydrogen; NTYPE=1 for para-H2 fraction fp
      END

      PROGRAM H2H2model
c     COPYRIGHT A.BORYSOW 1991 and 1993
...
c     (0-->1 vibrational transition) at temperatures less than 350K
...
c	A. Borysow, Icarus, vol.92, 273 (1991)
c	and erratum:  A. Borysow, Icarus, vol. 106, 614 (1993).
c	
        IMPLICIT double precision (A-H,O-Z)
	...
	common /wagi/ w(2)
 	common/ bb/         j1, j1p, j2, j2p, labeta
	common/ola/  xxm0
	common/sumrul/ g0, g1
        COMMON /SPECIT/ LIST
	common/ specit1/ FREQ(600), ABSCOEF(600), ALFATOT(600)
        COMMON/SPEKTRM/ FREQLO, DFREQ, FMAX, NTYPE,               
     1      Like, nvib_F, nvib_I
	...
	open(unit=10, file='output.h2h2',
     1                     status='unknown',form='formatted')

	write(*,*)  ' Which temperature? '
	read(*,*)  temp

c        Extrapolation of T range for consistency with other tables.
 	if ( (temp. lt. 20.) .or. (temp.gt.400.)) stop 1299
c	if ( (temp. lt. 20.) .or. (temp.gt.350.)) stop 1299
c	Temperatures should be within the range 20-350K

        write(*,*)  ' Which para-H2 fraction? 
     $       (put in negative number for equilibrium-H2)'
	read(*,*)  fp
...
C	DETERMINING THE PURELY VIBRATIONAL TRANSITION FREQUENCY:(CM^-1)
	...
	write(*, 6998) NVIB_I,NVIB_F, OMEGA_0
6998	FORMAT(' VIBRATIONAL TRANSITION FREQUENCY FOR ',I1,' --> ',
     1  I1,' IS EQUAL TO',/,F12.5,' CM-1')

c	write(*, 7799)
c7799	FORMAT(' The spectrum will be computed from FREQLO',
c     1   ' to FREQMAX',/,' FREQLO=?')
 	FREQLO=2000.d0
c	read(*,*) FREQLO
c	write(*, 1256)
c1256	format(' FREQMAX=?')
c	read(*,*) freqmax
        freqmax=8050.d0

c	write(*, 1266)
c1266	FORMAT(' Frequency STEP in cm^{-1} = ? ')
c	read(*,*) DELTASTEP
        DELTASTEP=50.d0

	list = INT (FREQMAX - FREQLO)/DELTASTEP
	IF(list.GT.nf) STOP 963
C	==>	CHANGE ' character nf = ' OR ADJUST DELTASTEP FOR A LARGER ONE!

	write(*, 1233) FREQLO, FREQMAX, DELTASTEP, list
...
10    ALFATOT(I)=0.
	...
	write(10, 1212) list, freqlo, deltastep
...
c	*****************************************************************
	labeta = '<0|B0001|1>'
c	ver. 1993
	XM0 = Z(temp, 0.66747d-61, -0.34043d+1, 0.62353d0, -0.27926d-01 ) 
	XM1 = Z(temp, 0.17096d-47, -0.33461d+1, 0.61460d0, -0.28201d-01 ) 
	g0 = xm0
	g1 = xm1
	XM2 = Z(temp, 0.94242d-34, -0.31984d+1, 0.52278d0, -0.12498d-01 ) 
...
c	ver. 1993
	omega1 = Z(temp, 0.27279d+2,-0.69452d0, 0.11661d0,-0.61575d-3 ) 
	omega2 = Z(temp, 0.11077d+2, 0.46570d-2, -0.49445d-1, 0.10357d-1)
	tauH =   Z(temp, 0.39182d-12, -0.52643d0, 0.39936d-5, 0.95064d-3) 

c     	 write(*,*) ' 0001 '
c       write(*,*) 'g0, g1, tau1, tau2, omega1, omega2, tauH'  
c       write(*,*) g0, g1, tau1, tau2, omega1, omega2, tauH
	...
	call range(ibgama)
        ...
        CALL ADDSPEC(TEMP,LIKE, 0, 0, 0, 1, freqcut,nvib_F,nvib_I,
     2  ibgama,  XM0, tau1,tau2, omega1, omega2,  tauH, fp )
...
c	1993 ver:
	XM0 = Z(temp,  0.74632d-64, -0.49090d0, 0.59048d-1, 0.89534d-2) 
	XM1 = Z(temp, 0.21555d-50,  -0.51456d0, 0.70540d-1, 0.72538d-2) 
	g0 = xm0
	g1 = xm1
	XM2 = z(temp, 0.93317d-37, -0.138170d0,-0.75397d-1, 0.26852d-1) 
...
c	parameters for correction term:
	omega1 = z(temp, 0.19834d+2, -0.44580d0,  0.60392d-1, 0.35041d-2 ) 
	omega2 = z(temp, 0.87918d+1, 0.19156d0, -0.92862d-1,  0.13604d-1 ) 
	tauH = z(temp, 0.37724d-12,  -0.51442d0, -0.21899d-2, 0.10429d-2 ) 

	ibgama = 0		! means, ==K0
	call range(0)
      CALL ADDSPEC(TEMP,LIKE, 0, 2, 2, 1, FMAX, nvib_F,nvib_I,
     2  ibgama,  XM0, tau1,tau2, omega1, omega2,  tauH, fp )
...
c	ver. 1993:
	omega1 = Z(temp,  0.27405d+2, -0.17864d+1, 0.39631d0, -0.19679d-1) 
	omega2 = Z(temp,  0.10116d+2, -0.74263d-2,-0.63547d-1, 0.12726d-1) 
	tauH = Z(temp,  0.55383d-12, -0.70431d0,  0.54877d-01,-0.42722d-2) 
...
c       write(*,*) g0, g1, tau1, tau2, omega1, omega2, tauH
	...
	ibgama = 1		! means, ==BC
	call range(1)
      CALL ADDSPEC(TEMP,LIKE, 0, 2, 2, 3, FMAX, nvib_F,nvib_I,
     2  ibgama,  XM0, tau1,tau2, omega1, omega2,  tauH, fp )
...
c	ver. 1993
	XM0 = Z(temp,  0.14553d-61,  -0.22892d1,   0.39167d0, -0.19424d-1) 
	XM1 = Z(temp,  0.12817d-48,  -0.24553d1,   0.42439d0, -0.20329d-1) 
	XM2 = Z(temp,  0.93224d-35,  -0.34191d1,   0.64311d0, -0.27519d-1) 
	g0 = xm0
	g1 = xm1
...
c	ver. 1993
	omega1 = Z(temp, 0.21178d2, -0.20090d1,   0.46603d0,  -0.25764d-1) 
	omega2 = Z(temp, 0.96463d1, 0.74521d-2, -0.74189d-1,   0.13590d-1) 
	tauH = Z(temp, 0.65879d-12, -0.76779d0,  0.71105d-1,  -0.51727d-2) 
...
c	write(*,*) g0, g1, tau1, tau2, omega1, omega2, tauH
	...
	ibgama = 1		! means, ==BC
	call range(1)
      CALL ADDSPEC( TEMP,LIKE, 2, 0, 2, 3, FMAX, nvib_F,nvib_I,
     2  ibgama,  XM0, tau1,tau2, omega1, omega2,  tauH, fp )
...
c	ver. 1993:
 	XM0 = Z(temp,  0.86252d-63, -0.22504d+01, 0.38397d0,  -0.19256d-1) 
	XM1 = Z(TEMP,  0.62723d-50, -0.23286d+1,  0.39342d0,  -0.18206d-1) 
	XM2 = Z(temp,  0.43604d-36, -0.33521d+1,  0.63337d0,  -0.27082d-1) 
	g0 = xm0
	g1 = xm1
...
c	ver. 1993:
	omega1 = Z(temp, 0.17717d+2,  -0.20101d+1, 0.47054d0, -0.26063d-1) 
	omega2 = Z(temp, 0.88423d+1,   0.10092d0,  -0.10153d0, 0.15948d-1) 
	tauH = Z(temp,   0.72376d-12, -0.84037d0, 0.92159d-1, -0.70604d-2) 

	ibgama = 1		! means, ==BC
	call range(1)
      CALL ADDSPEC(TEMP,LIKE, 2, 2, 3, 3, FMAX, nvib_F,nvib_I,
     2  ibgama,  XM0, tau1,tau2, omega1, omega2,  tauH, fp )
...
c	* * * * * * * * * * * ** * * * * * * * * * * ** * * * * * * * * * * *
	  ...
	  write(*,*)
	  write(*,*) ' ALL results in file: output.h2h2 '
	  write(*,*)
	  close(10)
	  open(unit=11, file='data', status='unknown')
	  write (11,9997) (freq(i), alfatot(i), i=1, list)
9997	  format(f10.2, e12.4)   
	  write(*,*) ' Final data: (freq, abs.coef) in file: data '
	  write(*,*)
...
11     continue
      ...
      SUBROUTINE ADDSPEC( TEMP,
     $     LIKE,LAMBDA1,LAMBDA2,LAMBDA,LVALUE, CUT, nvib_F, nvib_I,
     2  ibgama,  G0, tau1,tau2,  omega11, omega22, tauH, fp )
C
...
c	common/specit1/ FREQ(nf),ABSCOEF(nf), ALFATOT(nf)
   ...
   10 ABSCOEF(I)=0.D0
      ...
      BETA1 = 1.D0/(BOLTZWN*TEMP)
         ...
         WRITE (10,270) LAMBDA1,LAMBDA2,LAMBDA,LVALUE,JRANGE1,MARKER,
     1   W1(1),W1(2)
...
C      Molecule interacting with molecule (as opposed to atom):
	...
	XM00 =  G0	! pure quantum result 
      ...
      DO 160 IP1=1,JPLUSL
         ...
         IF (CG1S) 160,160,110
 110     if (fp .le. 0.) then   ! equilibrium
            P1=H2POPL(nvib_I,J1,TEMP,W1)/Q1
         else
            P1=H2POPL(nvib_I,J1,TEMP,W1)/QQ(TEMP, W1, J1)
...
C	"FIRST" H2 does not undergo vibrational transition
         ...
         DO 150 IP2=1,JPLUSL
            ...
            IF (CG2S) 150,150,120
 120        if (fp .le. 0.) then ! equilibrium
               P2=H2POPL(nvib_I,J2,TEMP,W1)/Q1
            else
               P2=H2POPL(nvib_I,J2,TEMP,W1)/QQ(TEMP, W1, J2)
...
C     NTYPE = 1 for specific para-H2 fraction, fp 
C
      IMPLICIT double precision (A-H,O-Z)
      CHARACTER*10  MARKER
      DIMENSION W(2)
...
C
C     for specific para-H2 fraction set w(2)=1
      if (NTYPE .eq. 1) then
         W(2)=1.
         MARKER='p-H2 frac '
      endif
...
C
   90  continue
       WRITE (10,310) marker, TEMP,Q,W(1),W(2)
  310  FORMAT (1x,a10, ' HYDROGEN',/,
     1  ' THE ROTATION PARTITION SUM OF H2 ',/,
     1 ' AT TEMPERATURE =', F8.2, 8H K IS Q=, E12.4,/,
     1  ' Weights J_even/odd:', 2F10.3,/ )
      RETURN
      END


      function QQ(T, W, J)
c     Computes the partition function for only para- or only ortho-H2
      IMPLICIT double precision (A-H,O-Z)
      DIMENSION W(2)
      DATA  BOLTZWN /.6950304D0/
      QQ=0.
      JP=mod(J,2)
 30   DQ=H2POPL(0,JP,T,W) + h2popl(1,JP,T,W) + 
     1     h2popl(2,JP,T,W) + h2popl(3,JP,T,W)       
      QQ = QQ + DQ
      JP = JP + 2
      IF (DQ.GT.QQ/990.) GO TO 30
      return
      end
...
c      common/specit1/ FREQ(nf),Abscoef(nf),Alfatot(nf)
      common/specit1/ FREQ(600),Abscoef(600),Alfatot(600)
      DIMENSION  XI(1),  aa(600), abs2(600), absp(600), fx(1)
...
13	continue
     ...
     1        /DFLOAT(1+LIKE)
      WRITE (K,330) FREQ(1),FREQ(LIST),FREQ(2)-FREQ(1),TEMP,
     1   LAMBDA1,LAMBDA2,LAMBDA,LVALUE,MESSAGE
  330 FORMAT (/,' ABSORPTION FROM',F10.1,' CM-1 TO',F7.1,' CM-1.',
     ...
     1  2X,4I2,5X,' IN UNITS OF CM-1 AMAGAT-2.',4X,A10/)
      WRITE (K,340) ( AA(I),I=1,LIST)
  340 FORMAT ( 110(6E12.4,/))
...
C     LGAS - 10 CHARACTER DESCRIPTION OF SYSTEM
	...
	go to 333
C 222    BBB=BETACOM(0.D0)       ! call BETA_JJ.for
 222    VVV=VACOMUN(0.D0)       ! call VA(R,k)
      RM(1)=RMN(1)/ANGAU
      EPS(1)=EPSP(1)
      RM(2)=RMN(2)/ANGAU
      EPS(2)=EPSP(2)
...
c	choosing YMAXX here:
	...
	if(ibgama.eq.1) gmax =g0 * bgama1(0.,tau1,tau2,temp)
	if(ibgama.eq.0) gmax =g0 * bgama0(0.,tau1,tau2,temp)
...
6565	YMAXXR = XS-STEP
	...
	XS = -100.D0
	if(ibgama.eq.1) gx =g0 * bgama1(0.,tau1,tau2,temp)
	if(ibgama.eq.0) gx =g0 * bgama0(0.,tau1,tau2,temp)
